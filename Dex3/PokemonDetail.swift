    ...
    ScrollView {
      ZStack {
        ...
        AsyncImage(url: showShiny ? pokemon.shiny : pokemon.sprite) { image in
          image.resizable().scaledToFit().shadow(color: .black, radius: 10)
        } placeholder: {
          ProgressView()
        }
      }
      ...
      HStack {
        ForEach(pokemon.types!, id: \.self) { type in
          Text(type.capitalized)
            ...
            .background(Color(type.capitalized)).cornerRadius(50)
        }
      ...
      Text("Stats").font(.title).padding([.bottom], -7)

      Stats().environmentObject(pokemon)

    }.navigationTitle(pokemon.name!.capitalized).toolbar {
